...
redis_error_throw(RedisSock *redis_sock)
{
    ...
    if (REDIS_SOCK_ERRCMP_STATIC(redis_sock, "ERR") &&
        !REDIS_SOCK_ERRCMP_STATIC(redis_sock, "ERR AUTH"))
        return;
                           ...
                           REDIS_SCAN_TYPE type, uint64_t *cursor)
{
    ...
    if (redis_read_reply_type(redis_sock, &reply_type, &reply_info) < 0 || reply_type != TYPE_MULTIBULK || reply_info != 2)
    {
    ...
    if (redis_read_reply_type(redis_sock, &reply_type, &reply_info) < 0 || reply_type != TYPE_BULK)
    {
        return -1;
    }
...
static zend_string *redis_hash_auth(zend_string *user, zend_string *pass)
{
    ...
    if (user)
        smart_str_append_ex(&salted, user, 0);
    if (pass)
        smart_str_append_ex(&salted, pass, 0);
...
redis_spprintf(RedisSock *redis_sock, short *slot, char **ret, char *kw, char *fmt, ...)
{
    ...
    while (*fmt)
    {
        ...
        case 'k':
            ...
            redis_cmd_append_sstr(&cmd, arg.str, arglen);
            if (slot)
                *slot = cluster_hash_key(arg.str, arglen);
            if (argfree)
                efree(arg.str);
            break;
        case 'v':
...
redis_boolean_response_impl(INTERNAL_FUNCTION_PARAMETERS, RedisSock *redis_sock,
                            zval *z_tab, void *ctx,
                            SuccessCallback_old success_callback)
{
...
redis_sock_check_liveness(RedisSock *redis_sock)
{
    ...
    if (auth)
    {
        ...
        else if (redis_strncmp(inbuf, ZEND_STRL("-NOAUTH")) == 0)
        {
             ...
             * fail too */
            if (redis_sock_gets(redis_sock, inbuf, sizeof(inbuf) - 1, &len) < 0 || redis_strncmp(inbuf, ZEND_STRL("-NOAUTH")) != 0)
            {
                goto failure;
            }
            return SUCCESS;
                       ...
                       int unserialize)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        int unwrap = ((unserialize == UNSERIALIZE_ALL) ||
