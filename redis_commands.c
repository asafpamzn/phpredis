...
redis_build_script_cmd(smart_string *cmd, int argc, zval *z_args)
{
    ...
    else if (zend_string_equals_literal_ci(Z_STR(z_args[0]), "flush"))
    {
        ...
        if (argc > 1 && (Z_TYPE(z_args[1]) != IS_STRING || (!zend_string_equals_literal_ci(Z_STR(z_args[1]), "sync") &&
                     ...
                     void **ctx)
{
    ...
    redis_cmd_init_sstr(&cmdstr, min_argc + !!opt.bylex + !!opt.byscore + !!opt.rev + !!opt.withscores + (opt.limit.enabled ? 3 : 0), kw, strlen(kw));
                       ...
                       char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    {
        if (argc < 1 || (zend_string_equals_literal_ci(Z_STR(argv[0]), "REPLACE") && argc < 2))
        {
            php_error_docref(NULL, E_WARNING, "argument required");
            return FAILURE;
        }
                    ...
                    char *kw, char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    redis_cmd_init_sstr(&cmdstr, 2 + (keys ? zend_hash_num_elements(keys) : 0) + (args ? zend_hash_num_elements(args) : 0), kw, strlen(kw));
                   ...
                   char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    if (blocking)
        redis_cmd_append_sstr_dbl(&cmdstr, timeout);
    redis_cmd_append_sstr_long(&cmdstr, zend_hash_num_elements(keys));
    ...
    ZEND_PARSE_PARAMETERS_START(3, 4){
        Z_PARAM_STR(key)
            Z_PARAM_LONG(timeout)
                Z_PARAM_STR(value)
                    Z_PARAM_OPTIONAL
                        Z_PARAM_ARRAY_HT_OR_NULL(options)} ZEND_PARSE_PARAMETERS_END_EX(return FAILURE);
                    ...
                    char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    ZEND_PARSE_PARAMETERS_START(3, 3){
        Z_PARAM_STR(src)
            Z_PARAM_STR(dst)
                Z_PARAM_ZVAL(zv)} ZEND_PARSE_PARAMETERS_END_EX(return FAILURE);
                   ...
                   char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    if (exp_type)
        redis_cmd_append_sstr_zstr(&cmdstr, exp_type);
    if (range_type)
        redis_cmd_append_sstr_zstr(&cmdstr, range_type);
                        ...
                        char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    /* Increment argc based on options */
    argc += gopts.withcoord + gopts.withdist + gopts.withhash + (gopts.sort != SORT_NONE) + (gopts.count ? 2 + gopts.any : 0);
    ...
    /* Append optional arguments */
    if (gopts.withcoord)
        REDIS_CMD_APPEND_SSTR_STATIC(&cmdstr, "WITHCOORD");
    if (gopts.withdist)
        REDIS_CMD_APPEND_SSTR_STATIC(&cmdstr, "WITHDIST");
    if (gopts.withhash)
        REDIS_CMD_APPEND_SSTR_STATIC(&cmdstr, "WITHHASH");
                             ...
                             char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    /* Increment argc based on options */
    argc += gopts.withcoord + gopts.withdist + gopts.withhash + (gopts.sort != SORT_NONE) + (gopts.count ? 2 : 0) + (gopts.store != STORE_NONE);
...
redis_build_client_list_command(smart_string *cmdstr, int argc, zval *z_args)
{
    ...
    if (argc > 0)
    {
        ...
        ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL(z_args[0]), zkey, z_ele)
        {
            if (zkey != NULL)
            {
                ...
                {
                    if (Z_TYPE_P(z_ele) != IS_STRING || (!ZVAL_STRICMP_STATIC(z_ele, "normal") &&
    ...
    REDIS_CMD_INIT_SSTR_STATIC(cmdstr, 1 + (type ? 2 : 0) + (id ? (Z_TYPE_P(id) == IS_ARRAY ? 1 + zend_hash_num_elements(Z_ARRVAL_P(id)) : 2) : 0), "CLIENT");
...
redis_build_client_kill_command(smart_string *cmdstr, int argc, zval *z_args)
{
    ...
    REDIS_CMD_INIT_SSTR_STATIC(cmdstr, 1 + (address != 0) + (id ? 2 : 0) + (type ? 2 : 0) + (user ? 2 : 0) + (addr ? 2 : 0) + (laddr ? 2 : 0) + (skipme ? 2 : 0), "CLIENT");
...
redis_build_client_tracking_command(smart_string *cmdstr, int argc, zval *z_args)
{
    ...
    REDIS_CMD_INIT_SSTR_STATIC(cmdstr, 2 + (redirect ? 2 : 0) + (prefix ? 2 * zend_hash_num_elements(Z_ARRVAL_P(prefix)) : 0) + bcast + optin + optout + noloop, "CLIENT");
    REDIS_CMD_APPEND_SSTR_STATIC(cmdstr, "TRACKING");
    if (Z_TYPE(z_args[0]) == IS_STRING && (ZVAL_STRICMP_STATIC(&z_args[0], "on") ||
                     ...
                     char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    else if (zend_string_equals_literal_ci(op, "CACHING"))
    {
        ...
        if (Z_TYPE(z_args[0]) == IS_STRING && (ZVAL_STRICMP_STATIC(&z_args[0], "yes") ||
    ...
    {
        if (argc < 1 || Z_TYPE(z_args[0]) != IS_LONG || (argc > 1 && (Z_TYPE(z_args[1]) != IS_STRING || (!ZVAL_STRICMP_STATIC(&z_args[1], "write") && !ZVAL_STRICMP_STATIC(&z_args[1], "all")))))
        {
            return FAILURE;
        }
    ...
    {
        if (argc > 0 && (Z_TYPE(z_args[0]) != IS_STRING || (!ZVAL_STRICMP_STATIC(&z_args[0], "on") &&
                                                            !ZVAL_STRICMP_STATIC(&z_args[0], "off") &&
    ...
    {
        if (argc < 1 || Z_TYPE(z_args[0]) != IS_STRING || (argc > 1 && (Z_TYPE(z_args[1]) != IS_STRING || (!ZVAL_STRICMP_STATIC(&z_args[1], "timeout") && !ZVAL_STRICMP_STATIC(&z_args[1], "error")))))
        {
            return FAILURE;
        }
        REDIS_CMD_INIT_SSTR_STATIC(&cmdstr, argc > 1 ? 3 : 2, "CLIENT");
                     ...
                     void **ctx)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "sss|l", &key, &keylen,
                              &start, &startlen, &end, &endlen, &count) == FAILURE)
    {
        return FAILURE;
    }
                         ...
                         char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "sssls|lb", &key, &keylen,
                              ...
                              &min_idle, &start, &startlen, &count, &justid) == FAILURE)
    {
        return FAILURE;
    }
                     ...
                     char **cmd, int *cmd_len, short *slot, void **ctx)
{
    ...
    if (nargs-- > 0)
        redis_cmd_append_sstr_zstr(&cmdstr, group);
    if (nargs-- > 0)
        redis_cmd_append_sstr_zstr(&cmdstr, id_or_consumer);
                     ...
                     void **ctx)
{
    ...
    redis_cmd_append_sstr_long(&cmdstr, timeout);
    if (mode != NULL)
        redis_cmd_append_sstr_zstr(&cmdstr, mode);
                             ...
                             RedisSock *redis_sock, redisCluster *c)
{
    ...
    case REDIS_OPT_SERIALIZER:
        ...
        if (val_long == REDIS_SERIALIZER_NONE || val_long == REDIS_SERIALIZER_PHP || val_long == REDIS_SERIALIZER_JSON
