...
/* Execute an UNLINK command using the Valkey Glide client - UNIFIED WITH DEL IMPLEMENTATION */
int execute_unlink_command(const void *glide_client, zval *keys, int keys_count, long *output_value)
{
    ...
    /* Detect single key vs multi-key scenario - SAME AS DEL COMMAND */
    ...
    {
        /* Multiple separate string arguments case: unlink('x', 'y', 'z') */
