...
/* {{{ proto array Redis::bzPopMax(string key [, string otherkeys, ...,], float timeout) */
PHP_METHOD(Redis, bzPopMax)
{
    ...
    if (redis->glide_client)
    {
        ...
        if (execute_bzpopmax_command(redis->glide_client, z_args, argc, timeout, return_value))
        {
...
/* {{{ proto array Redis::bzPopMin(string key [, string otherkeys, ...,], float timeout) */
PHP_METHOD(Redis, bzPopMin)
{
    ...
    if (redis->glide_client)
    {
        ...
        if (execute_bzpopmin_command(redis->glide_client, z_args, argc, timeout, return_value))
        {
