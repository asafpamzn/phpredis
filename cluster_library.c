                               ...
                               REDIS_REPLY_TYPE type)
{
    ...
    if (!CLUSTER_SEND_PAYLOAD(redis_sock, cmd, cmd_len) ||
        ...
        !redis_sock_gets_raw(redis_sock, buf, sizeof(buf)))
        return -1;
...
cluster_free(redisCluster *c, int free_ctx)
{
    ...
    if (c->flags->prefix)
        zend_string_release(c->flags->prefix);
                              ...
                              int direct)
{
    ...
    failover = c->readonly && c->failover != REDIS_FAILOVER_NONE ? c->failover : REDIS_FAILOVER_NONE;
    ...
    ZEND_HASH_FOREACH_PTR(c->nodes, seed_node)
    {
        ...
        if (seed_node == NULL || seed_node->sock == redis_sock || seed_node->slave)
            continue;
