...
int command_response_to_zval(CommandResponse *response, zval *output, int use_associative_array, bool use_false_if_null)
{
    ...
    case Array:
        ...
        else
        {
            for (int64_t i = 0; i < response->array_value_len; i++)
            {
                ...
                command_response_to_zval(&response->array_value[i], &value, use_associative_array, false);
    ...
    case Map:
        ...
        for (int i = 0; i < response->array_value_len; i++)
        {
            ...
            {
                command_response_to_zval(element->map_key, &key, use_associative_array, false);
            }
            else
            {
                ZVAL_NULL(&key);
            ...
            if (element->map_value != NULL)
            {
                ...
                command_response_to_zval(element->map_value, &value, use_associative_array, false);
...
int process_zrange_response(CommandResult *result, zval *return_value, int has_withscores)
{
    ...
    int res = command_response_to_zval(result->response, return_value, has_withscores ? COMMAND_RESPONSE_ASSOSIATIVE_ARRAY : COMMAND_RESPONSE_NOT_ASSOSIATIVE, false);
